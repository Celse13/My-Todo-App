...
const TodoList = () => {
  ...
  return (
      <div className="p-4 bg-white shadow-md rounded-lg mb-4 mt-4 flex-grow max-w-4xl mx-auto overflow-hidden">
        {data?.todos?.length === 0 ? (
            <EmptyState />
        ) : (
            data?.todos?.map((todo: todoType) => (
                <div key={todo.id} className="bg-white p-4 shadow-md rounded-lg mb-4 transition-transform transform hover:scale-105 max-w-full">
